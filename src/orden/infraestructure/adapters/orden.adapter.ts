...
export class ordenPersisteceAdapter implements iOrdenRepository {
  ...
  try {
    ...
    orden.numero_orden = ultimaOrden
      ? parseInt(ultimaOrden.numero_orden.toString()) + 1
      : 1;
  ...
  try {
    const pdtcb_odRecords = await this.pdtcb_odRepository.find({
      ...
      relations: [
        'producto',
        'registro_producto',
        'registro_producto.ingrediente',
      ],
    });

    const productos = pdtcb_odRecords.flatMap((pdtcb_od) => pdtcb_od.producto);
    const registrosProducto = pdtcb_odRecords.flatMap(
      (pdtcb_od) => pdtcb_od.registro_producto,
    );
    ...
    const costoIngredientes = registrosProducto.reduce(
      (total, registroProducto) => {
        const precio = registroProducto.precio
          ? parseFloat(registroProducto.precio)
          : 0;
        return total + precio;
      },
      0,
    ...
    const result = productos.map((producto, index) => {
      const registrosProductoProducto = registrosProducto.filter(
        (registroProducto) =>
          registroProducto.producto_id === producto.producto_id &&
          registroProducto.pdtcb_od_id === pdtcb_odRecords[index].pdtcb_od_id,
      );
      const ingredientes = registrosProductoProducto.map(
        (registroProducto) => registroProducto.ingrediente.nombre_ingrediente,
      );
      const precioIngredientes = registrosProductoProducto.reduce(
        (total, registroProducto) => {
          const precio = registroProducto.precio
            ? parseFloat(registroProducto.precio)
            : 0;
  ...
  async procesarOrdenId(
    orderId: createOrdenIdDto,
    orden: OrdenDto,
  ): Promise<any> {
  try {
    ...
    if (orden.numero_mesa) {
      const result = await this.ordenRepository.update(orderId.id, {
...
const ordenes = await this.ordenRepository.find({
    ...
    const resultado = ordenes.map((orden) => ({
      ...
      productos: orden.pdtcb_od.map((productoOrden) => {
        ...
        const detallesProducto = {
          ...
          tamano_producto: productoOrden.producto?.tamano_producto,
        ...
        const registrosProducto = productoOrden.registro_producto.filter(
          (registroProducto) => {
            return (
              registroProducto.producto_id === idProducto &&
              registroProducto.pdtcb_od_id === productoOrden.pdtcb_od_id
            );
          },
        );
        ...
        registrosProducto.forEach((registroProducto) => {
          detallesProducto.ingredientes.push({
            ...
            ingrediente_nombre:
              registroProducto.ingrediente?.nombre_ingrediente,
  ...
  try {
    if (pago.tipo_pago === 'electronico') {
      ...
      pagoElectronico.numero_referencia =
        pago.pagoElectronico.numero_referencia;
      ...
      if (pago.pagoEfectivo.length !== 0) {
        ...
        for (const pagoEfectivo of pago.pagoEfectivo) {
          ...
          const montoPagoEfectivo =
            parseFloat(pagoEfectivo.denomination) *
            parseFloat(pagoEfectivo.cantidadBilletes); // monto a agregar a la suma total
          ...
          newOrdenPagoPagoEfectivo.pagoefectivo_ordenpago_id =
            new UniqueId().getId();
          newOrdenPagoPagoEfectivo.orden_pago_id = newOrdenPago.orden_pago_id;
          newOrdenPagoPagoEfectivo.dolares_efectivo_id =
            newPagoEfectivo.dolares_efectivo_id;

          promises.push(
            this.ordenPago_pagoEfectivoRepository.save(
              newOrdenPagoPagoEfectivo,
            ),
          );
          
        }
        for (const pagoEfectivonew of pago.pagoEfectivo) {
        console.log('Monto tipo Test:', pago.pagoEfectivo[0].currency);
        if (pagoEfectivonew.currency === 'Bs.') {
          const montoDolares = totalPagoEfectivo / parseFloat(lastMontoBs);
          console.log('Monto montoDolares Test', montoDolares);
          this.ordenPagoRepository.update(newOrdenPago.orden_pago_id, {
            monto: montoDolares.toFixed(2).toString(),
            monto_dolares: montoDolares.toFixed(2).toString(),
          });
        } else {
          console.log('Monto totalPagoEfectivo Test', totalPagoEfectivo);
          this.ordenPagoRepository.update(newOrdenPago.orden_pago_id, {
            monto: totalPagoEfectivo.toFixed(2).toString(),
            monto_dolares: totalPagoEfectivo.toFixed(2).toString(),
          });
        }
      } 
      } else {
        ...
        if (pago.tipo_pago_efectivo === 'Bs.') {
          const montoDolares = parseFloat(pago.monto) / parseFloat(lastMontoBs);
          console.log('Monto montoDolares Test2', montoDolares);
          this.ordenPagoRepository.update(newOrdenPago.orden_pago_id, {
            monto_dolares: montoDolares.toFixed(2).toString(),
          });
        }
        ...
        promises.push(
          this.ordenPago_pagoEfectivoRepository.save(newOrdenPagoPagoEfectivo),
        );
      }
    ...
    try {
      const ordenes = await ordenPagoEntity.find({
        ...
        relations: [
          ...
          'zelle',
          'montoBs_Dolares',
          'ordenPago_pagoEfectivo',
          'ordenPago_pagoEfectivo.pagoEfectivo',
        ],
      });
      ...
      const pagos = ordenes.flatMap((orden) => {
        const tipoPago =
          orden.ordenPago_pagoEfectivo.length > 0
            ? 'efectivo'
            : orden.pagoElectronico
            ? 'electrónico'
            : orden.zelle
            ? 'zelle'
            : orden.montoBs_Dolares
            ? 'bsf'
            : null;

        const pagoEfectivoArray = orden.ordenPago_pagoEfectivo.reduce(
          (acc, ordenPago_pagoEfectivo) => {
            ...
            if (
              pagoEfectivo &&
              orden.fecha_historial === orden.fecha_historial
            ) {
              acc.push({
                pagoEfectivo,
  ...
  try {
    ...
    const lastMontos = montos.filter(
      (monto) => monto.fecha_historial === lastDate,
    );
    return lastMontos.map((monto) => ({
      monto: monto.monto,
      fecha_historial: monto.fecha_historial,
    }));
    } catch (error) {
      if (error instanceof HttpException) {
        throw error;
