...
CREATE TABLE OPCION_PROTEINA (
  id_opcion_proteina VARCHAR(100) NOT NULL,
  cantidad_opcion_proteina NUMERIC(5) NOT NULL,
  id_ingrediente VARCHAR(100) NOT NULL,
  id_producto VARCHAR(100) NOT NULL,
  CONSTRAINT pk_id_opcion_proteina PRIMARY KEY (id_opcion_proteina)
);
ALTER TABLE "cliente"
ADD CONSTRAINT "FK_d9cf8c718ba2133c20c14db42c3" FOREIGN KEY ("id_lugar_cliente") REFERENCES "lugar"("id_lugar") ON DELETE NO ACTION ON UPDATE NO action;
ALTER TABLE "lugar"
ADD CONSTRAINT "FK_40df63a39c51e4df499e3dfd87c" FOREIGN KEY ("id_padre_lugar") REFERENCES "lugar"("id_lugar") ON DELETE NO ACTION ON UPDATE NO action;
...
ALTER TABLE OPCION_PROTEINA
add constraint fk_id_opcion_proteina_ingrediente foreign key (id_ingrediente) references INGREDIENTE(ingrediente_id) ON DELETE CASCADE,
  add constraint fk_id_opcion_proteina_producto foreign key (id_producto) references PRODUCTO(producto_id) ON DELETE CASCADE;
